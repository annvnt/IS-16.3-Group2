...
Problem (Graham / Pinedo notation): FFc | bypass, b_i < inf | C_max
  FFc = Flexible Flow Shop with c stages in series, identical parallel
        machines within each stage (Pinedo, "Scheduling: Theory, Algorithms,
        and Systems", 5th ed., Ch. 1 & Ch. 16.3).
...
def run_experiments():
    ...
    # ── Explicit results table: Jobs, Stg, LB, FFLL, FFLL gap, IG-LS, IG-LS gap, Improv. ──
    col_titles = ["#", "Jobs", "Stg", "LB", "FFLL", "FFLL gap",
                  "IG-LS", "IG-LS gap", "Improv."]
    col_widths = [3, 6, 5, 8, 9, 10, 9, 11, 9]
    ...
    rule = "=" * sum(col_widths)
    print("\n" + rule)
    print("FFLL vs IG-LS — Results on 10 Random Instances")
    print(rule)
    ...
    print(fmt_row([
        "AVG", "-", "-", "-",
        "-", f"{np.mean(ffll_gaps):.1f}%",
        "-", f"{np.mean(igls_gaps):.1f}%",
        f"{np.mean(improvs):.1f}%",
    ]))
    print(rule)
...
if __name__ == "__main__":
    ...
    # ── Explicit standalone recap: Jobs | Stg | LB | FFLL | FFLL gap | IG-LS | IG-LS gap | Improv. ──
    print("\n" + "="*78)
    print("RECAP TABLE — Jobs | Stg | LB | FFLL | FFLL gap | IG-LS | IG-LS gap | Improv.")
    print("="*78)
    print(f"{'Jobs':>5} {'Stg':>4} {'LB':>7} {'FFLL':>8} {'FFLL gap':>10} "
          f"{'IG-LS':>8} {'IG-LS gap':>10} {'Improv.':>9}")
    print("-"*78)
    for r in results:
        print(f"{r['n_jobs']:>5} {r['n_stages']:>4} {r['lb']:>7.1f} "
              f"{r['ffll_ms']:>8.2f} {r['ffll_gap']:>9.1f}% "
              f"{r['igls_ms']:>8.2f} {r['igls_gap']:>9.1f}% {r['improv']:>8.1f}%")
    print("="*78)
